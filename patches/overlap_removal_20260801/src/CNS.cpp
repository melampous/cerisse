...
bool CNS::ib_momentum_budget = false;
bool CNS::ib_momentum_budget_viscous = false;
bool CNS::ib_momentum_budget_closed_surface = true;
int CNS::ib_momentum_budget_int = 1;
Real CNS::ib_momentum_budget_normal_tolerance = Real(1.0e-6);
std::string CNS::ib_momentum_budget_file = "ibm_momentum_budget.csv";
bool CNS::ib_momentum_budget_initialized = false;
Real CNS::ib_momentum_budget_previous_time = Real(0.0);
CNS::IBBudgetConserved CNS::ib_momentum_budget_previous_conserved{};
GpuArray<Real, AMREX_SPACEDIM> CNS::ib_momentum_budget_previous_force = {
    AMREX_D_DECL(Real(0.0), Real(0.0), Real(0.0))};
GpuArray<Real, AMREX_SPACEDIM>
    CNS::ib_momentum_budget_previous_pressure_force = {
        AMREX_D_DECL(Real(0.0), Real(0.0), Real(0.0))};
GpuArray<Real, AMREX_SPACEDIM>
    CNS::ib_momentum_budget_previous_viscous_force = {
        AMREX_D_DECL(Real(0.0), Real(0.0), Real(0.0))};
bool CNS::ib_momentum_budget_regrid_reference_valid = false;
Real CNS::ib_momentum_budget_regrid_reference_time = Real(0.0);
CNS::IBBudgetConserved
    CNS::ib_momentum_budget_regrid_reference_conserved{};
CNS::IBBudgetConserved CNS::ib_momentum_budget_regrid_delta_conserved{};
#endif
...
void CNS::read_params() {
  ...
  if (amr_state_interp_name == "linear") {
    ...
    amrex::Abort(
        "cns.amr_state_interp must be linear or conservative_quartic");
  }
  ...
  if (amr_state_interp != 0) {
    amrex::Abort(
        "higher-order cns.amr_state_interp modes are currently restricted to "
        "non-IBM, non-EB smooth-flow verification; pure GP-IBM retains the "
        "certified limited linear coarse-fine interpolation");
  }
#endif
  ...
  if (ibm_positivity_flux_limiter) {
...
#if (AMREX_SPACEDIM != 2)
    amrex::Abort(
        "cns.ibm_positivity_flux_limiter=1 is currently restricted to 2-D");
#endif
    ...
    amrex::Print()
        << "  cns.ibm_positivity_flux_limiter = 1 "
           ...
           "restricted Cartesian level-0 mode)\n";
  }
  ...
  if (ibm_positivity_retry) {
    amrex::Print()
        << "  cns.ibm_positivity_retry = 1 (transactional SSPRK43 trial "
           "rollback with binary time-interval subdivision; max_halvings="
        << ibm_positivity_max_step_halvings << ")\n";
  }
  ...
  ppib.query("momentum_budget", ib_momentum_budget);
  if (ib_momentum_budget) {
    ppib.query("momentum_budget_viscous", ib_momentum_budget_viscous);
    ppib.query("momentum_budget_closed_surface",
               ib_momentum_budget_closed_surface);
    ppib.query("momentum_budget_int", ib_momentum_budget_int);
    ppib.query("momentum_budget_normal_tolerance",
               ib_momentum_budget_normal_tolerance);
    ppib.query("momentum_budget_file", ib_momentum_budget_file);
    if (ib_move) {
      amrex::Abort("ib.momentum_budget currently supports fixed IBM geometry only");
    }
    if (ib_momentum_budget_int <= 0) {
      amrex::Abort("ib.momentum_budget_int must be positive");
    }
    if (order_rk == 0) {
      amrex::Abort(
          "ib.momentum_budget is incompatible with cns.order_rk=0 RHS-output mode");
    }
    if (!(ib_momentum_budget_normal_tolerance > Real(0.0)) ||
        !amrex::Math::isfinite(ib_momentum_budget_normal_tolerance)) {
      amrex::Abort("ib.momentum_budget_normal_tolerance must be finite and positive");
    }
    amrex::Print() << "  IBM full-domain conservation budget: interval="
                   << ib_momentum_budget_int
                   << " viscous_surface_force="
                   << static_cast<int>(ib_momentum_budget_viscous)
                   << " closed_surface="
                   << static_cast<int>(ib_momentum_budget_closed_surface)
                   << " normal_tolerance="
                   << ib_momentum_budget_normal_tolerance
                   << " file=" << ib_momentum_budget_file << "\n";
  }
#endif
...
void CNS::post_init(Real stop_time) {
...
#ifdef AMREX_USE_GPIBM
  if (plot_surf && !ib_move && !geom.IsRZ()) {
    {
      IBM::ib.beginConservativeSurfaceLoadWindow(
  ...
  if (ib_momentum_budget) {
    ...
    initializeIBMMomentumBudget(state[State_Type].curTime());
  }
#endif
...
#ifdef AMREX_USE_GPIBM
const iMultiFab* CNS::ibInterfaceUncoveredMask()
{
  const int finest = parent->finestLevel();
  if (level >= finest) {
    resetIBMInterfaceMask();
    return nullptr;
  }

  CNS& fine = getLevel(level + 1);
  const bool rebuild =
      !ib_interface_uncovered_mask ||
      ib_interface_uncovered_mask->boxArray() != grids ||
      ib_interface_uncovered_mask->DistributionMap() != dmap ||
      ib_interface_mask_fine_grids != fine.grids;
  if (rebuild) {
    ib_interface_uncovered_mask = std::make_unique<iMultiFab>(
        amrex::makeFineMask(
            grids, dmap, IntVect(1), fine.grids, parent->refRatio(level),
            geom.periodicity(), 1, 0));
    ib_interface_mask_fine_grids = fine.grids;
  }
  return ib_interface_uncovered_mask.get();
}

CNS::IBBudgetConserved CNS::compositeFluidConserved()
{
  AMREX_ALWAYS_ASSERT(level == 0);
  IBBudgetConserved conserved{};
  const GpuArray<int, ib_budget_nconserved> components{{
  ...
  for (int lev = 0; lev <= finest; ++lev) {
    CNS& level_ref = dynamic_cast<CNS&>(parent->getLevel(lev));
    MultiFab& state_mf = level_ref.get_new_data(State_Type);
    auto& marker_mf = *IBM::ib.bmf_a[lev];

    MultiFab volume(state_mf.boxArray(), state_mf.DistributionMap(), 1, 0,
                    MFInfo().SetArena(The_Async_Arena()));
    level_ref.geom.GetVolume(volume, state_mf.boxArray(),
                             state_mf.DistributionMap(), 0);

    std::unique_ptr<iMultiFab> uncovered;
    if (lev < finest) {
      CNS& fine_ref = dynamic_cast<CNS&>(parent->getLevel(lev + 1));
      uncovered = std::make_unique<iMultiFab>(amrex::makeFineMask(
          state_mf, fine_ref.grids, parent->refRatio(lev), 1, 0));
    }
    ...
    for (MFIter mfi(state_mf, false); mfi.isValid(); ++mfi) {
      ...
      auto const volume_array = volume.const_array(mfi);
      auto const marker = marker_mf.const_array(mfi);
      Array4<const int> uncovered_array;
      const bool has_fine_mask = static_cast<bool>(uncovered);
      if (has_fine_mask) uncovered_array = uncovered->const_array(mfi);
      ...
      reduce_op.eval(
          ...
          [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept -> ReduceTuple {
            const bool active =
                marker(i, j, k, 0) == 0 &&
                (!has_fine_mask || uncovered_array(i, j, k, 0) != 0);
            if (!active) {
              return {Real(0.0), Real(0.0), Real(0.0), Real(0.0),
                      Real(0.0)};
            }
            const Real cell_volume = volume_array(i, j, k, 0);
            return {state(i, j, k, components[0]) * cell_volume,
                    state(i, j, k, components[1]) * cell_volume,
                    state(i, j, k, components[2]) * cell_volume,
                    state(i, j, k, components[3]) * cell_volume,
                    state(i, j, k, components[4]) * cell_volume};
          });
    }
    const auto level_sum = reduce_data.value(reduce_op);
    conserved[0] += amrex::get<0>(level_sum);
    conserved[1] += amrex::get<1>(level_sum);
    conserved[2] += amrex::get<2>(level_sum);
    conserved[3] += amrex::get<3>(level_sum);
    conserved[4] += amrex::get<4>(level_sum);
  }
  ParallelDescriptor::ReduceRealSum(conserved.data(), ib_budget_nconserved);
  return conserved;
}

void CNS::accumulateIBMBudgetFlux(
    const std::array<MultiFab*, AMREX_SPACEDIM>& face_flux,
    Real stage_weight)
{
  if (!ib_momentum_budget || stage_weight == Real(0.0)) return;
  if (!(stage_weight > Real(0.0)) || !amrex::Math::isfinite(stage_weight)) {
    amrex::Abort(
        "IBM conservation budget requires a positive finite RK weight");
  }
  if (geom.IsRZ()) {
    amrex::Abort("IBM conservation budget face accumulation is Cartesian-only");
  }
  ...
  const GpuArray<int, ib_budget_nconserved> components{{
      ...
      PROB::ProbClosures::UET}};
  for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
    if (face_flux[dir] == nullptr) {
      amrex::Abort("IBM conservation budget received a null face flux");
    }
    const BoxArray expected =
        amrex::convert(grids, IntVect::TheDimensionVector(dir));
    if (face_flux[dir]->boxArray() != expected ||
        face_flux[dir]->DistributionMap() != dmap ||
        face_flux[dir]->nComp() != PROB::ProbClosures::NCONS) {
      amrex::Abort("IBM conservation budget face-flux layout mismatch");
    }
  }

  const auto dx = geom.CellSizeArray();
  GpuArray<Real, AMREX_SPACEDIM> face_area{};
#if (AMREX_SPACEDIM == 1)
  face_area[0] = Real(1.0);
#elif (AMREX_SPACEDIM == 2)
  face_area[0] = dx[1];
  face_area[1] = dx[0];
#else
  ...
  // Integrate only physical-boundary faces adjacent to an active fluid cell.
  // Solid cells have zero RHS and therefore must not contribute an outer flux.
  if (level == 0) {
    for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
      const IntVect ivd = IntVect::TheDimensionVector(dir);
      auto owner = face_flux[dir]->OwnerMask(geom.periodicity());
      Box low_face = amrex::surroundingNodes(domain, dir);
      low_face.setSmall(dir, domain.smallEnd(dir));
      low_face.setBig(dir, domain.smallEnd(dir));
      Box high_face = amrex::surroundingNodes(domain, dir);
      high_face.setSmall(dir, domain.bigEnd(dir) + 1);
      high_face.setBig(dir, domain.bigEnd(dir) + 1);

      for (MFIter mfi(*face_flux[dir], false); mfi.isValid(); ++mfi) {
        const Box valid_faces = mfi.validbox();
        const auto flux = face_flux[dir]->const_array(mfi);
        const auto marker = marker_mf.const_array(mfi);
        const auto owned = owner->const_array(mfi);
        for (int side = 0; side < 2; ++side) {
          const Box boundary_faces =
              valid_faces & (side == 0 ? low_face : high_face);
          if (!boundary_faces.ok()) continue;
          const Real orientation = side == 0 ? Real(-1.0) : Real(1.0);
          const bool low_side = side == 0;
          ReduceOps<ReduceOpSum, ReduceOpSum, ReduceOpSum, ReduceOpSum,
                    ReduceOpSum>
              reduce_op;
          ReduceData<Real, Real, Real, Real, Real> reduce_data(reduce_op);
          ...
          reduce_op.eval(
              ...
              [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
                ...
                const IntVect interior = low_side ? face : face - ivd;
                if (marker(interior, 0) != 0) {
                  return {Real(0.0), Real(0.0), Real(0.0), Real(0.0),
                          Real(0.0)};
                }
                const Real scale = orientation * face_area[dir];
                return {scale * flux(i, j, k, components[0]),
                        scale * flux(i, j, k, components[1]),
                        scale * flux(i, j, k, components[2]),
                        scale * flux(i, j, k, components[3]),
                        scale * flux(i, j, k, components[4])};
              });
          const auto sum = reduce_data.value(reduce_op);
          ib_boundary_conserved_impulse[0] +=
              stage_weight * amrex::get<0>(sum);
          ib_boundary_conserved_impulse[1] +=
              stage_weight * amrex::get<1>(sum);
          ib_boundary_conserved_impulse[2] +=
              stage_weight * amrex::get<2>(sum);
          ib_boundary_conserved_impulse[3] +=
              stage_weight * amrex::get<3>(sum);
          ib_boundary_conserved_impulse[4] +=
              stage_weight * amrex::get<4>(sum);
        }
      }
    }
  }

  // Every active fluid cell owns its outward fluid/solid crossing faces. A
  // cached uncovered mask removes cells covered by the next AMR level.
  const iMultiFab* uncovered = ibInterfaceUncoveredMask();
  const bool has_fine_mask = uncovered != nullptr;
  CNS& budget_owner = getLevel(0);
  ...
  for (MFIter mfi(marker_mf, false); mfi.isValid(); ++mfi) {
    ...
    reduce_op.eval(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept -> ReduceTuple {
          if (marker(i, j, k, 0) != 0 ||
              (has_fine_mask && uncovered_array(i, j, k, 0) == 0)) {
            return {Real(0.0), Real(0.0), Real(0.0), Real(0.0), Real(0.0),
                    Real(0.0)};
          }
          Real values[ib_budget_nconserved]{};
          Real absolute_mass_flux = Real(0.0);
          if (i > ilo && marker(i - 1, j, k, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] -=
                  face_area[0] * fx(i, j, k, components[n]);
            }
            absolute_mass_flux +=
                face_area[0] * amrex::Math::abs(fx(i, j, k, components[0]));
          }
          if (i < ihi && marker(i + 1, j, k, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] +=
                  face_area[0] * fx(i + 1, j, k, components[n]);
            }
            absolute_mass_flux += face_area[0] *
                amrex::Math::abs(fx(i + 1, j, k, components[0]));
          }
#if (AMREX_SPACEDIM >= 2)
          if (j > jlo && marker(i, j - 1, k, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] -=
                  face_area[1] * fy(i, j, k, components[n]);
            }
            absolute_mass_flux +=
                face_area[1] * amrex::Math::abs(fy(i, j, k, components[0]));
          }
          if (j < jhi && marker(i, j + 1, k, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] +=
                  face_area[1] * fy(i, j + 1, k, components[n]);
            }
            absolute_mass_flux += face_area[1] *
                amrex::Math::abs(fy(i, j + 1, k, components[0]));
          }
#endif
#if (AMREX_SPACEDIM == 3)
          if (k > klo && marker(i, j, k - 1, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] -=
                  face_area[2] * fz(i, j, k, components[n]);
            }
            absolute_mass_flux +=
                face_area[2] * amrex::Math::abs(fz(i, j, k, components[0]));
          }
          if (k < khi && marker(i, j, k + 1, 0) != 0) {
            for (int n = 0; n < ib_budget_nconserved; ++n) {
              values[n] +=
                  face_area[2] * fz(i, j, k + 1, components[n]);
            }
            absolute_mass_flux += face_area[2] *
                amrex::Math::abs(fz(i, j, k + 1, components[0]));
          }
#endif
    ...
    const auto sum = reduce_data.value(reduce_op);
    budget_owner.ib_interface_conserved_impulse[0] +=
        stage_weight * amrex::get<0>(sum);
    budget_owner.ib_interface_conserved_impulse[1] +=
        stage_weight * amrex::get<1>(sum);
    budget_owner.ib_interface_conserved_impulse[2] +=
        stage_weight * amrex::get<2>(sum);
    budget_owner.ib_interface_conserved_impulse[3] +=
        stage_weight * amrex::get<3>(sum);
    budget_owner.ib_interface_conserved_impulse[4] +=
        stage_weight * amrex::get<4>(sum);
    budget_owner.ib_interface_absolute_mass_impulse +=
        stage_weight * amrex::get<5>(sum);
  }
}

void CNS::auditIBMMomentumBudgetTopology()
{
  AMREX_ALWAYS_ASSERT(level == 0);
  if (geom.IsRZ()) {
    amrex::Abort(
        "ib.momentum_budget currently supports Cartesian geometry only; "
        "RZ requires swept IBM surface measures consistent with AMReX metrics");
  }
  if (parent->finestLevel() > 0 && !do_reflux) {
    amrex::Abort(
        "ib.momentum_budget with AMR requires cns.do_reflux=1; otherwise "
        "the composite momentum balance omits coarse-fine flux corrections");
  }

  // A Cartesian IBM face must have one AMR owner.  If a coarse-fine boundary
  // cuts between its fluid and solid cell centres, neither the coarse nor the
  // fine crossing flux alone is a well-defined representation of that wall
  // exchange.  Reject that hierarchy instead of silently double-counting or
  // omitting a body-force contribution.
  Long straddling_crossing_faces = 0;
  for (int lev = 0; lev < parent->finestLevel(); ++lev) {
    CNS& coarse = getLevel(lev);
    const iMultiFab* uncovered = coarse.ibInterfaceUncoveredMask();
    AMREX_ALWAYS_ASSERT(uncovered != nullptr);
    auto& marker_mf = *IBM::ib.bmf_a[lev];
    const Box domain = coarse.geom.Domain();
    ...
    for (MFIter mfi(marker_mf, false); mfi.isValid(); ++mfi) {
      const Box box = mfi.validbox();
      const auto marker = marker_mf.const_array(mfi);
      const auto owner = uncovered->const_array(mfi);
      reduce_op.eval(
          box, reduce_data,
          [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept -> ReduceTuple {
            const int here_owner = owner(i, j, k, 0);
            Long count = 0;
            if (i < domain.bigEnd(0) &&
                (marker(i, j, k, 0) != 0) !=
                    (marker(i + 1, j, k, 0) != 0) &&
                here_owner != owner(i + 1, j, k, 0)) {
              ++count;
            }
#if (AMREX_SPACEDIM >= 2)
    ...
    straddling_crossing_faces +=
        amrex::get<0>(reduce_data.value(reduce_op));
  }
  ParallelDescriptor::ReduceLongSum(straddling_crossing_faces);
  if (straddling_crossing_faces != 0) {
    amrex::Abort(
        "ib.momentum_budget found an IBM crossing face straddling an AMR "
        "coarse-fine ownership boundary; enlarge the body refinement buffer");
  }

  // The outer-flux accumulator uses level-0 physical faces.  Recheck this at
  // every sample because regridding can change the topology after startup.
  for (int lev = 1; lev <= parent->finestLevel(); ++lev) {
    CNS& fine = dynamic_cast<CNS&>(parent->getLevel(lev));
    const Box domain = fine.geom.Domain();
    for (int ibox = 0; ibox < fine.grids.size(); ++ibox) {
      const Box& box = fine.grids[ibox];
      for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
        if (box.smallEnd(dir) == domain.smallEnd(dir) ||
            box.bigEnd(dir) == domain.bigEnd(dir)) {
          amrex::Abort(
              "ib.momentum_budget requires all AMR fine grids to remain "
              "strictly inside the physical domain; composite fine-level "
              "outer-face fluxes are not implemented yet");
        }
      }
    }
  }
...
void CNS::refreshIBMMomentumRegridReference(Real time)
{
  if (!ib_momentum_budget || !ib_momentum_budget_initialized || level != 0) {
    return;
  }
  ib_momentum_budget_regrid_reference_conserved = compositeFluidConserved();
  ib_momentum_budget_regrid_reference_time = time;
  ib_momentum_budget_regrid_reference_valid = true;
}

void CNS::recordIBMMomentumRegridRemap(Real time)
{
  if (!ib_momentum_budget || !ib_momentum_budget_initialized || level != 0) {
    return;
  }
  if (!ib_momentum_budget_regrid_reference_valid) {
    amrex::Abort("IBM momentum budget has no pre-regrid momentum reference");
  }
  const Real time_scale = amrex::max(
      Real(1.0), amrex::max(amrex::Math::abs(time),
                            amrex::Math::abs(
                                ib_momentum_budget_regrid_reference_time)));
  if (amrex::Math::abs(
          time - ib_momentum_budget_regrid_reference_time) >
      Real(1.0e-12) * time_scale) {
    amrex::Abort(
        "IBM momentum budget pre/post-regrid states are at different times");
  }

  const auto post_regrid_conserved = compositeFluidConserved();
  Real momentum_delta_norm2 = Real(0.0);
  for (int component = 0; component < ib_budget_nconserved; ++component) {
    const Real delta = post_regrid_conserved[component] -
        ib_momentum_budget_regrid_reference_conserved[component];
    ib_momentum_budget_regrid_delta_conserved[component] += delta;
    ib_momentum_budget_regrid_reference_conserved[component] =
        post_regrid_conserved[component];
    if (component >= 1 && component <= 3) {
      momentum_delta_norm2 += delta * delta;
    }
  }
  ib_momentum_budget_regrid_reference_time = time;
  amrex::Print()
      << "  IBM conservation budget: recorded AMR remap delta_mass="
      << ib_momentum_budget_regrid_delta_conserved[0]
      << " |delta_M|=" << std::sqrt(momentum_delta_norm2)
      << " delta_energy=" << ib_momentum_budget_regrid_delta_conserved[4]
      << " at time=" << time << "\n";
}

void CNS::initializeIBMMomentumBudget(Real time)
{
  if (!ib_momentum_budget || level != 0) return;
  auditIBMMomentumBudgetTopology();

  ib_momentum_budget_previous_conserved = compositeFluidConserved();
  const auto surface =
      IBM::ib.integrateSurfaceForce(ib_momentum_budget_viscous);
  if (surface.nonfinite_faces != 0) {
    amrex::Abort("IBM momentum budget surface force contains nonfinite faces");
  }
  if (ib_momentum_budget_closed_surface &&
      surface.faces != static_cast<Long>(IBM::ib.ntotalfaces)) {
    amrex::Abort(
        "IBM momentum budget requires every surface face to have exactly one owner");
  }
  Real normal_norm2 = Real(0.0);
  for (int d = 0; d < 3; ++d) {
    normal_norm2 += surface.normal_closure[d] * surface.normal_closure[d];
  }
  const Real normal_relative = std::sqrt(normal_norm2) /
      amrex::max(surface.measure, Real(1.0e-300));
  if (!(surface.measure > Real(0.0)) ||
      !amrex::Math::isfinite(surface.measure) ||
      (ib_momentum_budget_closed_surface &&
       !(normal_relative <= ib_momentum_budget_normal_tolerance))) {
    amrex::Abort(
        "IBM momentum budget surface area/normal-closure audit failed");
  }
  for (int d = 0; d < AMREX_SPACEDIM; ++d) {
    ib_momentum_budget_previous_force[d] = surface.force[d];
    ib_momentum_budget_previous_pressure_force[d] =
        surface.pressure_force[d];
    ib_momentum_budget_previous_viscous_force[d] =
        surface.viscous_force[d];
  }
  ib_momentum_budget_previous_time = time;
  ib_momentum_budget_initialized = true;
  ib_boundary_conserved_impulse.fill(Real(0.0));
  ib_interface_conserved_impulse.fill(Real(0.0));
  ib_interface_absolute_mass_impulse = Real(0.0);
  ib_momentum_budget_regrid_delta_conserved.fill(Real(0.0));
  ib_momentum_budget_regrid_reference_conserved =
      ib_momentum_budget_previous_conserved;
  ib_momentum_budget_regrid_reference_time = time;
  ib_momentum_budget_regrid_reference_valid = true;

  if (ParallelDescriptor::IOProcessor()) {
    std::ofstream stream(ib_momentum_budget_file, std::ios::out | std::ios::trunc);
    if (!stream) {
      amrex::Abort("cannot open IBM momentum budget file " +
                   ib_momentum_budget_file);
    }
    stream
        << "step,time,dt,Mx,My,Mz,"
        << "regrid_delta_M_x,regrid_delta_M_y,regrid_delta_M_z,"
        << "dMdt_x,dMdt_y,dMdt_z,"
        << "outer_flux_x,outer_flux_y,outer_flux_z,"
        << "surface_force_x,surface_force_y,surface_force_z,"
        << "surface_pressure_force_x,surface_pressure_force_y,surface_pressure_force_z,"
        << "surface_viscous_force_x,surface_viscous_force_y,surface_viscous_force_z,"
        << "cv_conservative_force_x,cv_conservative_force_y,cv_conservative_force_z,"
        << "interface_force_x,interface_force_y,interface_force_z,"
        << "production_force_x,production_force_y,production_force_z,"
        << "production_load_source,"
        << "mapped_vtp_force_x,mapped_vtp_force_y,mapped_vtp_force_z,"
        << "mapped_vtp_interface_mismatch_x,mapped_vtp_interface_mismatch_y,"
        << "mapped_vtp_interface_mismatch_z,mapped_vtp_relative_mismatch,"
        << "mapped_vtp_force_valid,mapped_vtp_window_dt,"
        << "surface_force_mismatch_x,surface_force_mismatch_y,surface_force_mismatch_z,"
        << "surface_interface_mismatch_x,surface_interface_mismatch_y,surface_interface_mismatch_z,"
        << "interface_cv_mismatch_x,interface_cv_mismatch_y,interface_cv_mismatch_z,"
        << "closure_residual_x,closure_residual_y,closure_residual_z,"
        << "relative_residual,surface_measure,normal_closure_relative,"
        << "surface_faces,nonfinite_surface_faces,finest_level,reflux,viscous_force,"
        << "interface_force_valid,"
        << "mass,total_energy,regrid_delta_mass,regrid_delta_energy,"
        << "d_mass_dt,d_energy_dt,outer_mass_flux,outer_energy_flux,"
        << "interface_mass_flux,interface_absolute_mass_flux,interface_energy_flux,"
        << "mass_closure_residual,energy_closure_residual,"
        << "mass_relative_residual,energy_relative_residual,"
        << "discrete_momentum_residual_x,discrete_momentum_residual_y,"
        << "discrete_momentum_residual_z,discrete_relative_residual,"
        << "interface_conservation_valid\n";
  }
}

void CNS::updateIBMMomentumBudget(Real time)
{
  if (!ib_momentum_budget || level != 0) return;
  if (!ib_momentum_budget_initialized) {
    initializeIBMMomentumBudget(time);
    return;
  }
  auditIBMMomentumBudgetTopology();

  const Real dt_budget = time - ib_momentum_budget_previous_time;
  if (!(dt_budget > Real(0.0)) || !amrex::Math::isfinite(dt_budget)) {
    amrex::Abort("IBM momentum budget received a non-positive time interval");
  }

  const auto conserved = compositeFluidConserved();
  auto surface = IBM::ib.integrateSurfaceForce(ib_momentum_budget_viscous);
  if (surface.nonfinite_faces != 0) {
    amrex::Abort("IBM momentum budget surface force contains nonfinite faces");
  }
  if (ib_momentum_budget_closed_surface &&
      surface.faces != static_cast<Long>(IBM::ib.ntotalfaces)) {
    amrex::Abort(
        "IBM momentum budget lost unique ownership of one or more surface faces");
  }

  IBBudgetConserved boundary_impulse = ib_boundary_conserved_impulse;
  ParallelDescriptor::ReduceRealSum(
      boundary_impulse.data(), ib_budget_nconserved);
  IBBudgetConserved interface_impulse = ib_interface_conserved_impulse;
  ParallelDescriptor::ReduceRealSum(
      interface_impulse.data(), ib_budget_nconserved);
  Real interface_absolute_mass_impulse =
      ib_interface_absolute_mass_impulse;
  ParallelDescriptor::ReduceRealSum(interface_absolute_mass_impulse);
  const int interface_force_valid = 1;

  IBBudgetConserved regrid_delta{};
  IBBudgetConserved conserved_rate{};
  IBBudgetConserved outer_conserved_flux{};
  IBBudgetConserved interface_conserved_flux{};
  for (int component = 0; component < ib_budget_nconserved; ++component) {
    regrid_delta[component] =
        ib_momentum_budget_regrid_delta_conserved[component];
    conserved_rate[component] =
        (conserved[component] -
         ib_momentum_budget_previous_conserved[component] -
         regrid_delta[component]) /
        dt_budget;
    outer_conserved_flux[component] =
        boundary_impulse[component] / dt_budget;
    interface_conserved_flux[component] = interface_force_valid
        ? interface_impulse[component] / dt_budget
        : Real(0.0);
  }
  const Real interface_absolute_mass_flux =
      interface_absolute_mass_impulse / dt_budget;

  GpuArray<Real, 3> momentum3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> regrid_delta3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> dmdt3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> outer3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> force3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> pressure_force3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> viscous_force3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> conservative_force3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> interface_force3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> force_mismatch3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> surface_interface_mismatch3{
      {Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> interface_cv_mismatch3{
      {Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> residual3{{Real(0.0), Real(0.0), Real(0.0)}};
  GpuArray<Real, 3> discrete_residual3{
      {Real(0.0), Real(0.0), Real(0.0)}};
  Real dmdt_norm2 = Real(0.0);
  Real outer_norm2 = Real(0.0);
  Real force_norm2 = Real(0.0);
  Real residual_norm2 = Real(0.0);
  Real interface_norm2 = Real(0.0);
  Real discrete_residual_norm2 = Real(0.0);
  for (int d = 0; d < AMREX_SPACEDIM; ++d) {
    const int component = 1 + d;
    momentum3[d] = conserved[component];
    regrid_delta3[d] = regrid_delta[component];
    dmdt3[d] = conserved_rate[component];
    outer3[d] = outer_conserved_flux[component];
    force3[d] = Real(0.5) *
        (ib_momentum_budget_previous_force[d] + surface.force[d]);
    pressure_force3[d] = Real(0.5) *
        (ib_momentum_budget_previous_pressure_force[d] +
         surface.pressure_force[d]);
    viscous_force3[d] = Real(0.5) *
        (ib_momentum_budget_previous_viscous_force[d] +
         surface.viscous_force[d]);
    // This source-free control-volume force is conservative by construction.
    // Its difference from the reconstructed VTP traction is the quantity that
    // determines whether surface integration is trustworthy for total loads.
    conservative_force3[d] = -(dmdt3[d] + outer3[d]);
    interface_force3[d] = interface_conserved_flux[component];
    force_mismatch3[d] = force3[d] - conservative_force3[d];
    surface_interface_mismatch3[d] =
        force3[d] - interface_force3[d];
    interface_cv_mismatch3[d] =
        interface_force3[d] - conservative_force3[d];
    residual3[d] = dmdt3[d] + outer3[d] + force3[d];
    discrete_residual3[d] = interface_force_valid
        ? dmdt3[d] + outer3[d] + interface_force3[d]
        : std::numeric_limits<Real>::quiet_NaN();
    dmdt_norm2 += dmdt3[d] * dmdt3[d];
    outer_norm2 += outer3[d] * outer3[d];
    force_norm2 += force3[d] * force3[d];
    residual_norm2 += residual3[d] * residual3[d];
    interface_norm2 += interface_force3[d] * interface_force3[d];
    if (interface_force_valid) {
      discrete_residual_norm2 +=
          discrete_residual3[d] * discrete_residual3[d];
    }
  }
  const Real denominator = std::sqrt(dmdt_norm2) + std::sqrt(outer_norm2) +
                           std::sqrt(force_norm2);
  const Real relative_residual = std::sqrt(residual_norm2) /
      amrex::max(denominator, Real(1.0e-300));
  const Real discrete_denominator =
      std::sqrt(dmdt_norm2) + std::sqrt(outer_norm2) +
      std::sqrt(interface_norm2);
  const Real discrete_relative_residual = interface_force_valid
      ? std::sqrt(discrete_residual_norm2) /
            amrex::max(discrete_denominator, Real(1.0e-300))
      : std::numeric_limits<Real>::quiet_NaN();

  const Real nan = std::numeric_limits<Real>::quiet_NaN();
  GpuArray<Real, 3> mapped_vtp_force3{{nan, nan, nan}};
  GpuArray<Real, 3> mapped_vtp_interface_mismatch3{{nan, nan, nan}};
  Real mapped_vtp_relative_mismatch = nan;
  Real mapped_vtp_window_dt = nan;
  int mapped_vtp_force_valid = 0;
  if (IBM::ib.conservativeSurfaceForceMatchesWindow(
          ib_momentum_budget_previous_time, time)) {
    const auto& mapped = IBM::ib.lastConservativeSurfaceForceAudit();
    Real mapped_norm2 = Real(0.0);
    Real mapped_difference_norm2 = Real(0.0);
    for (int d = 0; d < 3; ++d) {
      mapped_vtp_force3[d] = mapped.force[d];
      mapped_vtp_interface_mismatch3[d] =
          mapped.force[d] - interface_force3[d];
      mapped_norm2 += mapped.force[d] * mapped.force[d];
      mapped_difference_norm2 +=
          mapped_vtp_interface_mismatch3[d] *
          mapped_vtp_interface_mismatch3[d];
    }
    mapped_vtp_relative_mismatch =
        std::sqrt(mapped_difference_norm2) /
        amrex::max(
            std::sqrt(mapped_norm2) + std::sqrt(interface_norm2),
            Real(1.0e-300));
    mapped_vtp_window_dt = mapped.window_dt;
    mapped_vtp_force_valid = 1;

    static const Real mapped_force_tolerance = [] {
      Real value = Real(1.0e-10);
      ParmParse pp("ib");
      pp.query("conservative_surface_force_tolerance", value);
      if (!(value > Real(0.0)) || !amrex::Math::isfinite(value)) {
        amrex::Abort(
            "ib.conservative_surface_force_tolerance must be finite and "
            "positive");
      }
      return value;
    }();
    if (!(mapped_vtp_relative_mismatch <= mapped_force_tolerance)) {
      amrex::Abort(
          "mapped VTP conservative force does not match the Cartesian "
          "interface force");
    }
  }

  const Real mass_residual = interface_force_valid
      ? conserved_rate[0] + outer_conserved_flux[0] +
            interface_conserved_flux[0]
      : std::numeric_limits<Real>::quiet_NaN();
  const Real energy_residual = interface_force_valid
      ? conserved_rate[4] + outer_conserved_flux[4] +
            interface_conserved_flux[4]
      : std::numeric_limits<Real>::quiet_NaN();
  const Real mass_relative_residual = interface_force_valid
      ? amrex::Math::abs(mass_residual) /
            amrex::max(amrex::Math::abs(conserved_rate[0]) +
                           amrex::Math::abs(outer_conserved_flux[0]) +
                           amrex::Math::abs(interface_conserved_flux[0]),
                       Real(1.0e-300))
      : std::numeric_limits<Real>::quiet_NaN();
  const Real energy_relative_residual = interface_force_valid
      ? amrex::Math::abs(energy_residual) /
            amrex::max(amrex::Math::abs(conserved_rate[4]) +
                           amrex::Math::abs(outer_conserved_flux[4]) +
                           amrex::Math::abs(interface_conserved_flux[4]),
                       Real(1.0e-300))
      : std::numeric_limits<Real>::quiet_NaN();
  Real normal_norm2 = Real(0.0);
  for (int d = 0; d < 3; ++d) {
  ...
  const Real normal_closure_relative = std::sqrt(normal_norm2) /
      amrex::max(surface.measure, Real(1.0e-300));
  if (!(surface.measure > Real(0.0)) ||
      !amrex::Math::isfinite(surface.measure) ||
      (ib_momentum_budget_closed_surface &&
       !(normal_closure_relative <= ib_momentum_budget_normal_tolerance))) {
    amrex::Abort(
        "IBM momentum budget surface area/normal-closure audit failed");
  }
  ...
  if (ParallelDescriptor::IOProcessor()) {
    std::ofstream stream(ib_momentum_budget_file, std::ios::out | std::ios::app);
    if (!stream) {
      amrex::Abort("cannot append IBM momentum budget file " +
                   ib_momentum_budget_file);
    }
    stream << std::setprecision(17)
           << step << ',' << time << ',' << dt_budget;
    for (Real value : momentum3) stream << ',' << value;
    for (Real value : regrid_delta3) stream << ',' << value;
    for (Real value : dmdt3) stream << ',' << value;
    for (Real value : outer3) stream << ',' << value;
    for (Real value : force3) stream << ',' << value;
    for (Real value : pressure_force3) stream << ',' << value;
    for (Real value : viscous_force3) stream << ',' << value;
    for (Real value : conservative_force3) stream << ',' << value;
    for (Real value : interface_force3) stream << ',' << value;
    // Fixed-body production load: exactly the equal-and-opposite momentum
    // exchange used by the finite-volume update.  The reconstructed STL
    // traction remains a separate local-wall accuracy diagnostic above.
    for (Real value : interface_force3) stream << ',' << value;
    stream << ",1";  // 1 = Cartesian crossing-flux production load
    for (Real value : mapped_vtp_force3) stream << ',' << value;
    for (Real value : mapped_vtp_interface_mismatch3) stream << ',' << value;
    stream << ',' << mapped_vtp_relative_mismatch
           << ',' << mapped_vtp_force_valid
           << ',' << mapped_vtp_window_dt;
    for (Real value : force_mismatch3) stream << ',' << value;
    for (Real value : surface_interface_mismatch3) stream << ',' << value;
    for (Real value : interface_cv_mismatch3) stream << ',' << value;
    for (Real value : residual3) stream << ',' << value;
    stream << ',' << relative_residual
           << ',' << surface.measure
           << ',' << normal_closure_relative
           << ',' << surface.faces
           << ',' << surface.nonfinite_faces
           << ',' << parent->finestLevel()
           << ',' << do_reflux
           << ',' << static_cast<int>(ib_momentum_budget_viscous)
           << ',' << interface_force_valid
           ...
           << ',' << conserved_rate[0]
           << ',' << conserved_rate[4]
           << ',' << outer_conserved_flux[0]
           << ',' << outer_conserved_flux[4]
           << ',' << interface_conserved_flux[0]
           << ',' << interface_absolute_mass_flux
           << ',' << interface_conserved_flux[4]
           << ',' << mass_residual
           << ',' << energy_residual
           << ',' << mass_relative_residual
           << ',' << energy_relative_residual;
    for (Real value : discrete_residual3) stream << ',' << value;
    stream << ',' << discrete_relative_residual
           << ',' << interface_force_valid
           << '\n';
  }

  ib_momentum_budget_previous_conserved = conserved;
  for (int d = 0; d < AMREX_SPACEDIM; ++d) {
    ib_momentum_budget_previous_force[d] = surface.force[d];
    ib_momentum_budget_previous_pressure_force[d] =
        surface.pressure_force[d];
  ...
  ib_interface_absolute_mass_impulse = Real(0.0);
  ib_momentum_budget_regrid_delta_conserved.fill(Real(0.0));
  ib_momentum_budget_regrid_reference_conserved = conserved;
  ib_momentum_budget_regrid_reference_time = time;
  ib_momentum_budget_regrid_reference_valid = true;
}
#endif
...
void CNS::postCoarseTimeStep(Real time) {
  ...
  const bool momentum_budget_due =
      ib_momentum_budget && (istep % ib_momentum_budget_int == 0);
  if (surface_output_due && !ib_move &&
             IBM::ib.conservativeSurfaceLoadWindowActive()) {
    const auto mapped =
        IBM::ib.finalizeConservativeSurfaceLoadWindow(time);
    if (ParallelDescriptor::IOProcessor()) {
      Print() << "  IBM conservative VTP load: window_dt="
              << mapped.window_dt << " crossings="
              << mapped.mapped_crossing_faces << " force=("
              << mapped.force[0] << "," << mapped.force[1] << ","
              << mapped.force[2] << ")\n";
    }
  }
#ifdef CNS_USE_FSI
  ...
  const bool surface_compute_due =
      surface_output_due || momentum_budget_due;
#endif
  ...
  if (momentum_budget_due) {
    updateIBMMomentumBudget(time);
  } else if (ib_momentum_budget) {
    // Regridding happens before the next coarse advance. Keep an exact
    // synchronized pre-remap reference even when the CSV sampling interval is
    // larger than one step; accumulated remap deltas remain untouched.
    refreshIBMMomentumRegridReference(time);
  }
...
void CNS::post_regrid(int lbase, int new_finest) {
  ...
  if (ib_momentum_budget && ib_momentum_budget_initialized &&
      level == new_finest) {
    CNS& base = dynamic_cast<CNS&>(parent->getLevel(0));
    base.recordIBMMomentumRegridRemap(state[State_Type].curTime());
  }
#endif  // AMREX_USE_GPIBM
...
#ifdef CNS_IBM_PROBLEM_SUPPORT_CAP
...
#endif
        bool support_refine_allowed = true;
#ifdef CNS_IBM_PROBLEM_SUPPORT_CAP
        // Some geometries need a spatially varying finest IBM level (for
        // example, a fine nozzle embedded in a much longer coarse aftbody).
        // This opt-in hook limits only the baseline interface seed; the usual
        // interpolation-order-derived dilation and strict support audit remain
        // active.  Cases that do not define CNS_IBM_PROBLEM_SUPPORT_CAP retain
        // the original whole-interface behaviour bit for bit.
...
#endif
        support_refine_allowed = PROB::ibm_support_refine_allowed(
...
#ifdef AMREX_USE_GPIBM
  rebuildIBM();
  if (level == 0 && plot_surf && !ib_move && !geom.IsRZ()) {
    {
      IBM::ib.beginConservativeSurfaceLoadWindow(
          state[State_Type].curTime());
    }
  ...
  if (output_due || force_compute)  {
    ...
    if (output_due && this->level == parent->finestLevel()){
      ...
      if (amrex::ParallelDescriptor::IOProcessor()){
        IBM::ib.projectConservativeSurfaceLoads();
