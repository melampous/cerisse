...
// NPR_0 = 4.0  (p_0/p_a = 3)  =>  pe/pa ~ 2.113
...
struct ProbParm
{
  ...
  // NPR_0 = 4.0 => pe/pa = NPR_0 / (1+0.2)^3.5 = 3.0 / 1.89293
  Real p_jet    = 101325.0 * 4.0 / 1.89293;
  Real T_jet    = 300.0;
  Real Mach_jet = 1.0;
                  ...
                  const ProbParm& pparm, int level)
{
  ...
  if (level == 0) {
    tagfab(i, j, k) = tagfab(i, j, k) || (grad_rho > Real(0.04));
  } else {
    tagfab(i, j, k) = tagfab(i, j, k) || (grad_rho > Real(0.08));
  }
}
