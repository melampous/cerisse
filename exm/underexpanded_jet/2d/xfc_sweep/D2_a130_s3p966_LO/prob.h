...
struct ProbParm
{
  ...
  Real delta_jet = 1.300000e-04;  // delta/De = 0.01, tanh lip layer
...
void inline inputs() {
  ...
  const Real r0 = p.r_jet - 3.965528*p.delta_jet;
  const int  N  = 20000;
         ...
         ProbParm const& pparm)
{
  ...
  if (r < pparm.r_jet) {
    ...
    const Real r0 = pparm.r_jet - Real(3.965528) * pparm.delta_jet;
    Real fjet;
