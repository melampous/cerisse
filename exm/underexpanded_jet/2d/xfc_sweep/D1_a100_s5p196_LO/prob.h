...
struct ProbParm
{
  ...
  Real delta_jet = 1.000000e-04;  // delta/De = 0.01, tanh lip layer
...
void inline inputs() {
  ...
  const Real r0 = p.r_jet - 5.195590*p.delta_jet;
  const int  N  = 20000;
         ...
         ProbParm const& pparm)
{
  ...
  if (r < pparm.r_jet) {
    ...
    const Real r0 = pparm.r_jet - Real(5.195590) * pparm.delta_jet;
    Real fjet;
