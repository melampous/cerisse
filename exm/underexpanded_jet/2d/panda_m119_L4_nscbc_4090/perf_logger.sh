...
D="$(cd "$(dirname "$0")" && pwd)"
OUT="$D/perf4090.csv"
echo "utc,step,t_ms,dt,s_per_step,eta_h,gpu_T,gpu_W,vram_MiB,util,freeRAM_MiB" > "$OUT"
PREV_STEP=0; PREV_EPOCH=$(date +%s)
while pgrep -f "main2d.gnu.MPI.CUDA.e[x]" > /dev/null; do
  sleep 300
  L=$(grep -E "^STEP = " "$D/run4090b.log" 2>/dev/null | tail -1)
  ...
  if [ -n "$STEP" ] && [ "$STEP" -gt "$PREV_STEP" ] 2>/dev/null; then
    ...
    ETA=$(awk -v t="$T" -v dt="$DT" -v s="$SPS" 'BEGIN{if(dt>0&&s>0)printf "%.1f",(10.0e-3-t)/dt*s/3600; else print "nan"}')
  fi
