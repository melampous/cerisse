...
// NPR_0 = 2.0  (p_0/p_a = 3)  =>  pe/pa ~ 5.283
...
struct ProbParm
{
  ...
  // NPR_0 = 2.0 => pe/pa = NPR_0 / (1+0.2)^3.5 = 3.0 / 1.89293
  Real p_jet    = 101325.0 * 3.2735 / 1.89293;   // Mj=1.42 (Panda & Seasholtz 1999)
  Real T_jet    = 300.0;
  Real Mach_jet = 1.0;

  Real r_jet     = 0.005;         // 5 mm, D_e = 10 mm
  Real delta_jet = 100.0e-6;      // finite lip layer
};
                  ...
                  const ProbParm& pparm, int level)
{
  ...
  // near-field (RZ: i->r, j->z axial); weak NPR=2 jet -> lowered thresholds
  ...
  const bool nearfield = (xz < Real(0.035)) && (xr < Real(0.010));
  // weak jet (no shocks): per-cell gradient metric halves per level for smooth
  // waves, so the cascade self-limits -> force the near-field box to L6 unconditionally
  if (level == 0) {
    tagfab(i, j, k) = tagfab(i, j, k) || nearfield || (grad_rho > Real(0.03));
  } else if (level < 3) {
    tagfab(i, j, k) = tagfab(i, j, k) || nearfield || (grad_rho > Real(0.05));
  } else {
    tagfab(i, j, k) = tagfab(i, j, k) || nearfield;
  }
}
