...
NUMERICS_DIR = SCRIPT_DIR.parent
...
def plot_vortex_accuracy(
    ...
    figure, axes = plt.subplots(1, 2, figsize=(7.05, 3.20))
    ...
    axes[0].set_ylabel(
        r"Density error, $\|\rho-\rho_{\mathrm{exact}}\|_2$"
    )
    axes[1].set_ylabel(
        r"Perturbation energy loss, "
...
def plot_field_matrix(
    ...
    for column, ((label, vorticity), (_, leaf_field)) in enumerate(
        ...
        top_axis.set_title(label, pad=6.0)
    ...
    top_cbar_axis = figure.add_axes((0.925, 0.565, 0.014, 0.305))
    bottom_cbar_axis = figure.add_axes((0.925, 0.145, 0.014, 0.305))
    ...
    top_cbar.set_label(r"$\omega_z R_v/u_\infty$")
    bottom_cbar.set_label(
        r"$100(\rho-\rho_{\mathrm{exact}})/\rho_\infty$"
    )
...
def main() -> None:
    ...
    default_output = SCRIPT_DIR / "y9000x_three_scheme_20260727"
