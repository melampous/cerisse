...
DEFAULT_OUTPUT_DIR = (
    SCRIPT_DIR / "y9000x_mms_convergence_20260727"
)
...
VARIABLES = (
    ...
    ("y_momentum", r"$\rho u_y$ or $\rho u_r$", "#009E73", "^"),
...
def make_figure(
    ...
    figure = plt.figure(figsize=(7.05, 5.85))
    ...
    rz_axis = figure.add_subplot(grid[1, :])
    ...
    rz_axis.loglog(
        ...
        zorder=4,
        label=(
            r"Semidiscrete $\rho u_r$ residual, "
            r"first-ring $L_\infty$"
        ),
    )
    add_order_guide(
        rz_axis,
    ...
    for axis in (euler_axis, ns_axis, rz_axis):
        style_axis(axis)
