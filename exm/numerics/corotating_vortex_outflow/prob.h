...
//  OUTFLOW VARIANT: lateral (x=±4) boundaries use foextrap (code 2),
...
//  reflection. This variant tests foextrap's non-reflecting property.
//
...
struct ProbParm {
  ...
  // foextrap is selected by setting cns.lo_bc/hi_bc = 2 2 in inputs
  // (AMReX handles it; bcnormal is then never called for those faces).
  // NOTE: In this OUTFLOW variant, inputs.outflow sets:
  //   cns.lo_bc = 2 7     (x-lo: foextrap, y-lo: characteristic)
  //   cns.hi_bc = 2 7     (x-hi: foextrap, y-hi: characteristic)
  int  bc_mode  = 1;
  int  use_lodi = 0;       // 1 -> enable bcnormal_lodi (for code-7 faces only)
...
// ====================================================================
// bcnormal: OUTFLOW VARIANT - called ONLY on code-7 (characteristic)
// faces, which in inputs.outflow are y-lo/hi boundaries.
//
// Target state is the quiescent background (rho0, 0, 0, p0) on y faces.
// x-direction faces (x=±4) use foextrap (code 2), which AMReX handles
// directly without calling bcnormal.
//
...
//
// NOTE: In this OUTFLOW variant, LODI would only apply to y-faces
// (y=±4), since x-faces use foextrap directly via AMReX.
// ====================================================================
AMREX_GPU_DEVICE AMREX_FORCE_INLINE
