...
struct LESparm_t {
  ...
  static constexpr Real Cs = 0.16;         // Smagorinsky constant (STANDARD, SGS ON)
...
struct viscparm_t {
  ...
  static constexpr Real viscosity    = 0.0;   // infinite Re => SGS-only dissipation
  static constexpr Real conductivity = 0.0;
};

typedef closures_dt<indicies_t, transport_const_t<viscparm_t>,
                    calorifically_perfect_gas_t<indicies_t>, Smagorinsky_t<LESparm_t,indicies_t>>
    ProbClosures;
// 6th-order pure central CONVECTIVE flux + explicit Smagorinsky SGS via viscousLES
typedef rhs_dt<centraldif_t<false,false,6,ProbClosures>, viscousLES_t<viscparm_t, ProbClosures>, no_source_t>
    ProbRHS;
