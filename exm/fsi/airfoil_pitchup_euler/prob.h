...
// Pitch-up trajectory: airfoil stationary until t_settle, then constant ω
namespace MotionPitch {
  AMREX_GPU_MANAGED inline Real t_settle    = 720.0e-6;   // [s] flow-develop time (≈5 chord at M=2)
  AMREX_GPU_MANAGED inline Real omega_pitch = -2330.0;  // negative: CW → LE up (pitch-up AoA)     // [rad/s] constant pitch rate (k≈0.17)
}
...
{
  RigidState s{};
  s.dx = s.dy = s.vx = s.vy = 0.0_rt;
  if (time < MotionPitch::t_settle) {
    s.theta = 0.0_rt;
    s.omega = 0.0_rt;
  } else {
    s.theta = MotionPitch::omega_pitch * (time - MotionPitch::t_settle);
    s.omega = MotionPitch::omega_pitch;
  }
  return s;
}
...
struct ProbParm
{
  // Supersonic freestream M=2 (left-to-right inflow)
  static constexpr Real Ma_oo   = 2.0;
...
// RHS: WENO-Z5 + Euler (no diffusive)
...
void prob_initdata (int i, int j, int k, amrex::Array4<amrex::Real> const& state,
  ...
  // Initialize entire field QUIESCENT at rest
  //   u = v = 0, ρ = ρ_amb, p = p_amb, T = T_amb
  // Freestream M=2 enters only through x-low BC (bcnormal).
  Real rho = pp.rho_oo;
...
#endif
  state(i, j, k, cls.UET)  = rho * pp.eint_oo;   // no kinetic energy initially
}
...
void user_tagging(int i, int j, int k, int nt, auto& tagfab, const auto &sdatafab,
  ...
  // Solid-vicinity tag: refine all 4 levels (max_level=3 → L1,L2,L3 nested toward IB)
  if (level < 4) {
     bool foundGP = false;
