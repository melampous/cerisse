...
# ---------------------------------------------------------------------------
# Slice-only 3D readers — load only 3 orthogonal planes (memory-efficient)
# ---------------------------------------------------------------------------

def read_amrex_3d_multi_slices(pltdir, requested_vars, ix, iy, iz,
                               out_level=-1, prolong_method="linear"):
    """Read 3 orthogonal slices of multiple variables without loading the full 3D field.

    Memory usage: O(Ny*Nz + Nx*Nz + Nx*Ny) instead of O(Nx*Ny*Nz).

    Parameters
    ----------
    ix, iy, iz : int
        Cell indices at the target (finest output) level for each slice plane.

    Returns (slices_dict, x_coords, y_coords, z_coords, dx, dy, dz, meta).
    slices_dict[varname] = (slice_x, slice_y, slice_z) where
      slice_x[j, k] = field(ix, j, k)   shape (Ny, Nz)
      slice_y[i, k] = field(i, iy, k)   shape (Nx, Nz)
      slice_z[i, j] = field(i, j, iz)   shape (Nx, Ny)
    """
    header = _parse_header_3d(pltdir)
    nvars = header["nvars"]
    varnames = header["varnames"]
    ...
    # Allocate only the three 2D slice planes per variable
    slices = {}
    for name in requested_vars:
        slices[name] = (
            np.zeros((Ny, Nz), dtype=np.float64),   # slice_x at ix
            np.zeros((Nx, Nz), dtype=np.float64),   # slice_y at iy
            np.zeros((Nx, Ny), dtype=np.float64),    # slice_z at iz
        )
    ...
    for lev in range(target_level + 1):
        ...
        try:
            for b in range(len(boxes)):
                ...
                # Fine-level index range covered by this box
                fi_lo = blo_i * ratio
                fi_hi = (bhi_i + 1) * ratio - 1
                fj_lo = blo_j * ratio
                fj_hi = (bhi_j + 1) * ratio - 1
                fk_lo = blo_k * ratio
                fk_hi = (bhi_k + 1) * ratio - 1

                # Skip boxes that don't intersect any slice plane
                hit_x = (fi_lo <= ix <= fi_hi)
                hit_y = (fj_lo <= iy <= fj_hi)
                hit_z = (fk_lo <= iz <= fk_hi)
                if not (hit_x or hit_y or hit_z):
                    continue
                ...
                for name, vidx in var_to_idx.items():
                    patch = fab[vidx].transpose(2, 1, 0)  # -> (ni, nj, nk)
                    ...
                    if hit_x:
                        loc = ix - fi_lo
                        slices[name][0][fj_lo:fj_hi+1, fk_lo:fk_hi+1] = fine[loc, :, :]

                    if hit_y:
                        loc = iy - fj_lo
                        slices[name][1][fi_lo:fi_hi+1, fk_lo:fk_hi+1] = fine[:, loc, :]

                    if hit_z:
                        loc = iz - fk_lo
                        slices[name][2][fi_lo:fi_hi+1, fj_lo:fj_hi+1] = fine[:, :, loc]
    ...
    return slices, x_coords, y_coords, z_coords, dx, dy, dz, meta


def read_amrex_3d_multi_slices_multi_ix(pltdir, requested_vars,
                                         ix_list=(), iy_list=(), iz_list=(),
                                         out_level=-1, prolong_method="linear"):
    """Read multiple parallel slices in each direction in a SINGLE pass over boxes.

    Optimised for cases with many slice planes per variable.
    Box-skip + multi-slice extraction in one disk pass.

    Parameters
    ----------
    ix_list, iy_list, iz_list : iterable of int
        Cell indices at finest output level for each set of slice planes.

    Returns (slices_dict, xc, yc, zc, dx, dy, dz, meta), where
        slices_dict[varname] = (
            { ix:  np.ndarray (Ny, Nz) },          # YZ slices
            { iy:  np.ndarray (Nx, Nz) },          # XZ slices
            { iz:  np.ndarray (Nx, Ny) },          # XY slices
        )
    """
    ix_list = sorted(set(int(v) for v in ix_list))
    iy_list = sorted(set(int(v) for v in iy_list))
    iz_list = sorted(set(int(v) for v in iz_list))
    ...
    # Allocate per-variable per-direction dictionaries of slice arrays
    slices = {}
    for name in requested_vars:
        slices[name] = (
            {ix: np.zeros((Ny, Nz), dtype=np.float64) for ix in ix_list},
            {iy: np.zeros((Nx, Nz), dtype=np.float64) for iy in iy_list},
            {iz: np.zeros((Nx, Ny), dtype=np.float64) for iz in iz_list},
        )
    ...
    for lev in range(target_level + 1):
        ...
        try:
            for b in range(len(boxes)):
                ...
                # Pick which slices this box contributes to
                hits_x = [ix for ix in ix_list if fi_lo <= ix <= fi_hi]
                hits_y = [iy for iy in iy_list if fj_lo <= iy <= fj_hi]
                hits_z = [iz for iz in iz_list if fk_lo <= iz <= fk_hi]
                if not (hits_x or hits_y or hits_z):
                    continue
                ...
                for name, vidx in var_to_idx.items():
                    ...
                    slx_dict, sly_dict, slz_dict = slices[name]
                    for ix in hits_x:
                        loc = ix - fi_lo
                        slx_dict[ix][fj_lo:fj_hi+1, fk_lo:fk_hi+1] = fine[loc, :, :]
                    for iy in hits_y:
                        loc = iy - fj_lo
                        sly_dict[iy][fi_lo:fi_hi+1, fk_lo:fk_hi+1] = fine[:, loc, :]
                    for iz in hits_z:
                        loc = iz - fk_lo
                        slz_dict[iz][fi_lo:fi_hi+1, fj_lo:fj_hi+1] = fine[:, :, loc]
...
def read_amrex_3d_slices(pltdir, varname, ix, iy, iz,
                         out_level=-1, prolong_method="linear"):
    """Read 3 orthogonal slices of a single variable (memory-efficient).

    Returns (slice_x, slice_y, slice_z, x_coords, y_coords, z_coords, dx, dy, dz, meta).
    """
    slices_dict, xc, yc, zc, dx, dy, dz, meta = read_amrex_3d_multi_slices(
        pltdir, [varname], ix, iy, iz, out_level, prolong_method)
    sx, sy, sz = slices_dict[varname]
    return sx, sy, sz, xc, yc, zc, dx, dy, dz, meta
...
# ---------------------------------------------------------------------------
# Slice-only loaders for derived quantities (memory-efficient 3D)
# ---------------------------------------------------------------------------

def _load_mach_3d_slices(pltdir, ix, iy, iz, gamma=1.4,
                         out_level=-1, prolong_method="linear"):
    """Load Mach-number slices from a 3D plotfile (memory-efficient)."""
    header = _parse_header_3d(pltdir)
    varnames = header["varnames"]
    vset = {v.lower() for v in varnames}
    vmap = {v.lower(): v for v in varnames}
    ...
    if "x_velocity" in vset and "y_velocity" in vset and "z_velocity" in vset:
        need = [vmap["density"], vmap["pressure"],
                vmap["x_velocity"], vmap["y_velocity"], vmap["z_velocity"]]
        slices, xc, yc, zc, ddx, ddy, ddz, meta = read_amrex_3d_multi_slices(
            pltdir, need, ix, iy, iz,
            out_level=out_level, prolong_method=prolong_method)
        rho = slices[vmap["density"]]
        p   = slices[vmap["pressure"]]
        u   = slices[vmap["x_velocity"]]
        v   = slices[vmap["y_velocity"]]
        w   = slices[vmap["z_velocity"]]
    elif "xmom" in vset and "ymom" in vset and "zmom" in vset:
        need = [vmap["density"], vmap["pressure"],
                vmap["xmom"], vmap["ymom"], vmap["zmom"]]
        slices, xc, yc, zc, ddx, ddy, ddz, meta = read_amrex_3d_multi_slices(
        ...
        rho = slices[vmap["density"]]
        p   = slices[vmap["pressure"]]
        xmom = slices[vmap["xmom"]]
        ymom = slices[vmap["ymom"]]
        zmom = slices[vmap["zmom"]]
        # Convert momentum to velocity on each slice
        u = tuple(xmom[k] / np.maximum(rho[k], eps) for k in range(3))
        v = tuple(ymom[k] / np.maximum(rho[k], eps) for k in range(3))
        w = tuple(zmom[k] / np.maximum(rho[k], eps) for k in range(3))
    else:
        raise ValueError(
            "Mach requires (x_velocity,y_velocity,z_velocity) or "
            "(Xmom,Ymom,Zmom) in plotfile"
    ...
    # Compute Mach per slice
    mach_slices = []
    for k in range(3):
        speed = np.sqrt(u[k] * u[k] + v[k] * v[k] + w[k] * w[k])
        a = np.sqrt(gamma * np.maximum(p[k], 0.0) / np.maximum(rho[k], eps))
        mach_slices.append(speed / np.maximum(a, eps))

    return tuple(mach_slices), xc, yc, zc, ddx, ddy, ddz, meta


def _load_plot_field_3d_slices(pltdir, varname, ix, iy, iz, gamma=1.4,
                               out_level=-1, prolong_method="linear"):
    """Load 3 orthogonal slices of a raw or derived 3D field (memory-efficient).

    Returns ((slice_x, slice_y, slice_z), x, y, z, dx, dy, dz, meta, label).
    """
    vlow = varname.strip().lower()
    if vlow in ("mach", "mach_number"):
        s, xc, yc, zc, ddx, ddy, ddz, meta = _load_mach_3d_slices(
            pltdir, ix, iy, iz, gamma, out_level, prolong_method=prolong_method)
        return s, xc, yc, zc, ddx, ddy, ddz, meta, "Mach"

    sx, sy, sz, xc, yc, zc, ddx, ddy, ddz, meta = read_amrex_3d_slices(
        pltdir, varname, ix, iy, iz, out_level, prolong_method=prolong_method)
    return (sx, sy, sz), xc, yc, zc, ddx, ddy, ddz, meta, varname
...
def _main_3d(args):
    ...
    # ── Resolve grid metadata and slice origin BEFORE loading data ─────
    print(f"Reading {plotfile} ...", flush=True)
    header = _parse_header_3d(plotfile)
    _fl = header["finest_level"]
    _tl = _fl if args.level < 0 else min(args.level, _fl)
    _fbox = header["all_boxes"][_tl]
    _Nx = int(_fbox[3]) - int(_fbox[0]) + 1
    _Ny = int(_fbox[4]) - int(_fbox[1]) + 1
    _Nz = int(_fbox[5]) - int(_fbox[2]) + 1
    _dx = (header["hi"][0] - header["lo"][0]) / _Nx
    _dy = (header["hi"][1] - header["lo"][1]) / _Ny
    _dz = (header["hi"][2] - header["lo"][2]) / _Nz
    _xc = header["lo"][0] + (np.arange(_Nx) + 0.5) * _dx
    _yc = header["lo"][1] + (np.arange(_Ny) + 0.5) * _dy
    _zc = header["lo"][2] + (np.arange(_Nz) + 0.5) * _dz

    if args.origin is not None:
        ox, oy, oz = args.origin
    else:
        ox = 0.5 * (header["lo"][0] + header["hi"][0])
        oy = 0.5 * (header["lo"][1] + header["hi"][1])
        oz = 0.5 * (header["lo"][2] + header["hi"][2])

    ix = int(np.argmin(np.abs(_xc - ox)))
    iy = int(np.argmin(np.abs(_yc - oy)))
    iz = int(np.argmin(np.abs(_zc - oz)))

    # ── Load only the 3 slice planes (memory-efficient) ───────────────
    try:
        (slice_x, slice_y, slice_z), xc, yc, zc, dx, dy, dz, meta, var_label = \
            _load_plot_field_3d_slices(
                plotfile, varname, ix, iy, iz,
                args.gamma, args.level, prolong_method=args.prolong,
            )
    except ValueError as exc:
    ...
    # Diagnostics on slice data
    all_slice_vals = np.concatenate([slice_x.ravel(), slice_y.ravel(), slice_z.ravel()])
    finite = all_slice_vals[np.isfinite(all_slice_vals)]
    if finite.size > 0:
        print(f"  {var_label} range (slices): [{finite.min():.6g}, {finite.max():.6g}]")
    else:
        print(f"  {var_label}: all slice cells are NaN/inf")

    # ── Extract 2D slices (already 2D, just transpose to match plot axes)
    slice_front = slice_x.T   # (Nz, Ny)
    slice_side  = slice_y.T   # (Nz, Nx)
    slice_top   = slice_z.T   # (Ny, Nx)
    ...
    # ── Solid mask (slice-only) ────────────────────────────────────────
    mask_front = mask_side = mask_top = None
    if args.mask_solid:
        try:
            sld_x, sld_y, sld_z, _, _, _, _, _, _, _ = read_amrex_3d_slices(
                plotfile, "sld", ix, iy, iz, args.level,
                prolong_method="nearest")
            mask_front = (sld_x.T != 0)
            mask_side  = (sld_y.T != 0)
            mask_top   = (sld_z.T != 0)
    ...
    # ── Plot (width ratios proportional to physical extent) ─────────────
    # Front: horiz=Y, vert=Z;  Side: horiz=X, vert=Z;  Top: horiz=X, vert=Y
    Ly = yc[-1] - yc[0] + dy
    Lx = xc[-1] - xc[0] + dx
    Lz = zc[-1] - zc[0] + dz
    w_front, w_side, w_top = Ly, Lx, Lx       # horizontal physical extents
    h_front, h_side, h_top = Lz, Lz, Ly       # vertical physical extents
    max_h = max(h_front, h_side, h_top)
    w_ratios = [w_front, w_side, w_top]

    if args.figsize:
        figsize = tuple(args.figsize)
    else:
        # Scale so the tallest panel is ~5 inches, total width follows ratios
        panel_h = 5.0
        total_w = sum(w_ratios) / max_h * panel_h
        # Add padding for labels, suptitle, and colorbar
        figsize = (total_w + 2.5, panel_h + 1.2)
