#ifndef IBM_SHARED_GP_FLUX_UTILS_H_
#define IBM_SHARED_GP_FLUX_UTILS_H_

// Marker, stencil, and Cartesian flux helpers shared by the frozen full-cell
// GP-IBM production path and optional IBM extensions.
...
one_sided_polynomial_flux(const amrex::IntVect& face, const int dir,
                          ...
                          const Cls& cls,
                          const bool use_rz_radial_metric = false,
                          const amrex::Real radial_origin = amrex::Real(0.0),
                          const amrex::Real radial_spacing = amrex::Real(1.0))
    noexcept
{
  using amrex::Real;
  const amrex::IntVect ivd = amrex::IntVect::TheDimensionVector(dir);

  Real face_radius = Real(1.0);
  if (use_rz_radial_metric) {
    face_radius = radial_origin + Real(face[0]) * radial_spacing;
  ...
  for (int m = 0; m < count; ++m) {
    const amrex::IntVect cell = face + (start + m) * ivd;
    cls.prims2flux(cell, dir, prims, cell_flux);
    if (use_rz_radial_metric) {
      const Real cell_radius =
          radial_origin + (Real(cell[0]) + Real(0.5)) * radial_spacing;
      const Real metric_scale = cell_radius / face_radius;
      for (int n = 0; n < Cls::NCONS; ++n) {
        value[n] += weight[m] * metric_scale * cell_flux[n];
      }
    } else {
      for (int n = 0; n < Cls::NCONS; ++n) {
