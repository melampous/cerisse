#!/usr/bin/env python3
"""Structure loss across the five formulations, on one slice.

Rows are LLF+WENO-Z5, LLF+TENO5, AFD-HLLC+WENO-Z5, AFD-HLLC+TENO5 and
skew-symmetric+JST, so the two flux paths sit as adjacent pairs, the
reconstruction alternates inside each pair, and the formulation that belongs to
neither factor comes last. Every figure uses one colour bar for all four rows, so a difference
between rows is a difference in the data and never in the mapping. Same grid,
same slice plane, same normalisation.

    struct5_vorticity.png   |omega| D_e/U_j, full view and the shear layer
    struct5_schlieren.png   instantaneous numerical schlieren
    struct5_lambda2.png     lambda_2 of the mean-square strain-rotation tensor
    struct5_q.png           Q criterion
...
Instantaneous density is now on disk for all five, taken from the same frame
as the velocities, so the schlieren is a true instantaneous one and does show
contact surfaces and the braids between rollers. The mean-density version is
kept alongside it: the two answer different questions, the instantaneous one
about resolved structure and the averaged one about shock-cell sharpness.
...
def stack4(key, cmap, lo, hi, cbar, out, light_text=True, zoom=False):
    n = 10 if zoom else 5
    fig, AX = plt.subplots(n, 1, figsize=(9.4, 10.4 if not zoom else 15.6),
                           sharex=False)
...
stack4("omg", "magma", 0.0, 25.0, r"$|\omega|\,D_e/U_j$",
       "struct5_vorticity", zoom=True)
stack4("sch", "gray", 0.0, 1.0, "instantaneous numerical schlieren,   "
       r"$\exp(-k|\nabla\rho|/g_{ref})$",
       "struct5_schlieren", light_text=False)
...
stack4("lam2", "RdBu_r", -80.0, 80.0,
       r"$\lambda_2\,(D_e/U_j)^2$", "struct5_lambda2", light_text=False)
stack4("Q", "RdBu_r", -80.0, 80.0, r"$Q\,(D_e/U_j)^2$", "struct5_q",
       light_text=False)
