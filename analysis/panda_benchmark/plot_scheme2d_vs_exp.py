#!/usr/bin/env python3
"""2D-RZ L4 scheme pair (LLF WENO-Z5 + 4th-order viscous vs skew+JST) against
Panda & Seasholtz (1999), centreline density. Both legs share the identical
grid, inlet, protocol and statistics window [2, 6] ms.
...
x2, r2, lo2, hi2, T = series2d("skew2d_axis")
xw, rw, low, hiw, Tw = series2d("llfweno2d_axis")
...
Q = {}
for name, xs, rs, tend in [("2D-RZ L4 LLF WENO-Z5", xw, rw, Tw[-1]),
                           ("2D-RZ L4 skew+JST",    x2, r2, T[-1])]:
...
for lab, xx, rr in [("weno", xw, rw), ("skew", x2, r2)]:
    w = (xx >= XLO) & (xx <= 7.0)
    xs, rs = xx[w], rr[w]
    ...
    for k in range(min(len(ie), len(isim))):
        print("  %-4s %-3d %9.3f %9.3f %+9.3f %+9.2f"
              % (lab, k+1, xe[ie[k]], xs[isim[k]], xs[isim[k]]-xe[ie[k]],
                 100*(xs[isim[k]]-xe[ie[k]])/xe[ie[k]]))
    print("  %s spacing sim  %s" % (lab, " ".join("%.3f" % v for v in np.diff(xs[isim]))))
...
ww = (xw >= 0.0) & (xw <= 7.2)
a1.fill_between(xw[ww], low[ww], hiw[ww], color="#0b6ef5", alpha=0.18, lw=0,
                zorder=3)
a1.plot(xw[ww], rw[ww], color="#0b6ef5", ls="-", lw=2.0, zorder=5,
        label="2D-RZ $L4$ LLF WENO-Z5 (4th-order viscous),  window [2, 6] ms")
w2 = (x2 >= 0.0) & (x2 <= 7.2)
a1.plot(x2[w2], r2[w2], color="#009E73", ls="--", lw=1.4, zorder=4,
...
bandw = np.interp(xe, xw, hiw-low)
a2.fill_between(xe, -0.5*bandw, 0.5*bandw, color="0.55", alpha=0.28, lw=0,
                zorder=2, label="statistical spread of the mean")
a2.plot(xe, Q["2D-RZ L4 LLF WENO-Z5"], color="#0b6ef5", lw=2.0, zorder=5,
        label="LLF WENO-Z5")
a2.plot(xe, Q["2D-RZ L4 skew+JST"], color="#009E73", ls="--", lw=1.4, zorder=4,
        label="skew+JST")
...
fig.savefig(HERE/"scheme2d_vs_exp_centreline.png", dpi=250)
fig.savefig(HERE/"scheme2d_vs_exp_centreline.pdf")
print("\nwrote scheme2d_vs_exp_centreline.png/pdf")
