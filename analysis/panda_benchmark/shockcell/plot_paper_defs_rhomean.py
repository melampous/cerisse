...
(a) Unsmoothed centreline mean density of the 3D L5 solution, with the
    density-based operational closure x_fc and the density-based principal
    compression locations x_n marked.
(b) Time-mean density field of the same solution, with x_fc, x_n and the
...
# ---- density-based x_fc and x_n from the 3D L5 mean ----
...
rhos = boxfilt(rho, xrho, 0.05)
dist = max(1, int(round(0.5 / (xrho[1]-xrho[0]))))
pk, _ = find_peaks(np.where(xrho > xfc, rhos, -np.inf),
                   prominence=0.02, distance=dist)
xn = [refine(xrho, rhos, i) for i in pk[:4]]
print("xfc=%.3f  xn=%s  Sn=%s" % (xfc, ["%.3f" % v for v in xn],
                                  ["%.3f" % s for s in np.diff(xn)]))

# ---- mean density field (same z=0 plane) ----
sm = np.load(HERE / "SIM_L5_rhoMEAN_z0.npz")
SHL = sm["rho"] / RHO_J
dxs = float(sm["x"][1] - sm["x"][0])
...
im = ax1.imshow(SHL.T, origin="lower", extent=[xs[0], xs[-1], ys[0], ys[-1]],
           vmin=0.4, vmax=1.5, cmap="RdBu_r", aspect="equal",
           interpolation="nearest", rasterized=True)
cb = fig.colorbar(im, ax=ax1, pad=0.012, fraction=0.035)
cb.set_label("$\\langle\\rho\\rangle/\\rho_j$", labelpad=3, fontsize=10)
...
fig.savefig(HERE / "paperfig_metricdefs_rhomean.png", dpi=300)
fig.savefig(HERE / "paperfig_metricdefs_rhomean.pdf")
print("wrote paperfig_metricdefs_rhomean")
