...
// 2D Axisymmetric (RZ) underexpanded jet — quasi-steady NPR sweep (2026-07-20).
...
{
  constexpr Real plat[NPR_NPLAT] =
      {1.89293, 2.394, 2.800, 3.273446, 3.671, 4.200, 4.900, 5.746};
  Real npr = plat[0];
  for (int k = 0; k < NPR_NPLAT - 1; ++k) {
    const Real tc = NPR_TC0 + Real(k) * NPR_DTC;
    npr += (plat[k+1] - plat[k])
           * Real(0.5) * (Real(1.0) + std::tanh((t - tc) / NPR_TAU));
  }
  return npr;
}
