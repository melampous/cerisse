...
run_dir="${case_root}/testH_soft_Tmin_continuation"
...
test -f "${restart}/Level_0/SD_0_New_MF_H"
test -f "${restart}/Level_1/SD_0_New_MF_H"
test -f "${restart}/Level_2/SD_0_New_MF_H"
test -f "${geometry}"
...
printf '%s\n' "$$" > launcher_pid.txt
{
  ...
  printf 'run_class=nonconservative_temperature_floor_rescue\n'
  printf 'production_validation=0\n'
  printf 'temperature_floor_K=10\n'
  printf 'cfl=0.01\n'
  ...
  printf 'soft_positivity=1\n'
  printf 'soft_floor_semantics=preserve_momentum_raise_total_energy\n'
...
if (( now_epoch >= deadline_epoch )); then
  printf '%s\n' 'ERROR: continuation requested after capacity-block safety deadline' \
    > run_state.txt
  exit 3
fi
