...
run_dir=${run_root}/testD_final_source_chk03400_step4000_normal_regrid
restart=${run_root}/testC_final_source_chk03370_step3400/output/run165/chk03400
...
printf '%s\n' running > run_state.txt
{
  ...
  printf 'positivity_flux_limiter=0\n'
  printf 'temperature_floor_injection=disabled\n'
  printf 'plot_int=25\n'
  printf 'checkpoint_int=100\n'
...
if [[ "${status}" -eq 0 && "${final_step}" -ge "${target_step}" && \
      "${failure_count}" -eq 0 && \
