...
run_dir=${run_root}/testC_final_source_chk03370_step3400
restart=${run_root}/testB_chk03300_regrid_step3370/output/run165/chk03370
...
printf '%s\n' running > run_state.txt
{
  ...
  printf 'host=%s\n' "$(hostname)"
  printf 'restart=%s\n' "${restart}"
  printf 'target_step=3400\n'
  printf 'regrid_on_restart=0\n'
...
if [[ "${status}" -eq 0 && "${final_step}" -ge 3400 && \
      "${failure_count}" -eq 0 && \
